...
## Loading the data from "household_power_consumption.txt" to plot4data (in reference to the assignment)
...
plot4data <- read.table("household_power_consumption.txt", header = T, sep = ";", na.strings = "?")

## Checking the dimension of the data set plot4data
dim(plot4data)
## [1] 2075259       9
...
## Checking the initial content of the plot4data data set. 
head(plot4data)
...
plot4subdata <- subset(plot4data, subset = (Date >= "2007-02-01" & Date <= "2007-02-02"))
...
plot4subdata$datetime <- strptime(paste(plot4subdata$Date, plot4subdata$Time), "%Y-%m-%d %H:%M:%S")
...
plot4subdata$datetime <- as.POSIXct(plot4subdata$datetime)

## Declaring the graphical parameters
## Setting 2 cloumns and 2 rows for the presentation of the graphs
par(mfrow = c(2, 2))

## Plotting the Global Active Power over the plot4subdata$datetime values
## X-label (xlab) is set to "" as per expected output of the assignment
## if xlab is not declared it will output the entire subset "plot4subdata$datetime as its label.
plot(Global_active_power ~ plot4subdata$datetime, type = "l", ylab = "Global Active Power", xlab = "")

## Plotting the Voltage data over the plot4subdata$datetime values
## Adding xlab = "datetime" as expected output of the assignment.
plot(Voltage ~ plot4subdata$datetime, type = "l", xlab = "datetime")

## Plotting Sub_metering_1 over the values of plot4subdata$datetime.
## X-label (xlab) is set to "" as per expected output of the assignment
plot(Sub_metering_1 ~ plot4subdata$datetime, type = "l", ylab = "Energy sub metering", xlab = "")
## Adding lines to the graph having Sub_metering_2 over the values of plot4subdata$datetime. Using the color "Red" 
lines(Sub_metering_2 ~ plot4subdata$datetime, col = "Red")
## Adding lines to the graph having Sub_metering_3 over the values of plot4subdata$datetime. Using the color "Blue" 
lines(Sub_metering_3 ~ plot4subdata$datetime, col = "Blue")
...
## Plotting the Global Reactive Power over the plot4subdata$datetime subset.
## Global Reactive Power is 4th object from the data set. 
plot(Global_reactive_power ~ plot4subdata$datetime, type = "l", xlab = "datetime")


## Saving the plotted historgram as "plot4.png"
